    ...
    def compute(self):
        ...
        while timeIdx < len(self.timeSlices):
            ...
            while layerIdx < len(self.layerHeights):
                ...
                electronDensity.append(sumLayers(self.atmosphere.cells[timeIdx]['electronDensity'],
                                                 2,
                                                 0,
                                                 upperLayerHeight,
                                                 heights))
                refractiveIndex.append(np.sqrt(1. - refractiveScale*electronDensity[-1]/self.getFrequency()**2))
                fineRefractiveIndex = np.sqrt(1. - refractiveScale*self.atmosphere.cells[timeIdx]['electronDensity']/self.getFrequency()**2)
