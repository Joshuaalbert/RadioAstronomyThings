...
if __name__=='__main__':
    try:
        ...
        if len(obj) == 2:
            ra = float(obj[0])
            dec = float(obj[1])
            getSolarTransit("%f,%f"%(ra,dec),lon,lat,time,tzOffset=-5)
            ...
            try:
                ...
                getSolarTransit("%f,%f"%(ra,dec),obj,lon,lat,time,tzOffset=-5)
                #getTransit(lon,lat,time,ra,dec,tzOffset=+1.)

            except:
                try:
                    getSolarTransit(obj,lon,lat,time,tzOffset=-5)
                except:
                    try:
                        ra = float(obj.split(',')[0])
                        dec = float(obj.split(',')[1]) 
                        getTransit(lon,lat,time,ra,dec,tzOffset=-5)
                    except:
                        ra = 0.712305555
                        dec = 41.2694444
                        getTransit(lon,lat,time,ra,dec,tzOffset=-5)
