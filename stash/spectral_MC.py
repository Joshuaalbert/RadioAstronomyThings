...
def doPixelMC(S_array,error_array,nu_array,calError=0.2,nu_ref=150e6,order=None,ax = None, M=1000,title=''):
    ...
    while i < np.size(mask):
        if mask[i]:
            ax.errorbar(nu_array[i],S_array[i]*nu_array[i],yerr=error_array[i]*nu_array[i],fmt='o',color='red')
        else:
    ...
    curylim = ax.get_ylim()
    ax.set_ylim([min(curylim[0],max(0.,np.min(S_array*nu_array)*0.5)),max(curylim[1],np.max(S_array*nu_array)*1.5)])
...
if __name__ == '__main__':
   ...
   # print S_test
    ...
    if True:
        ...
        ax2.set_xscale('log')
        ax2.set_yscale('log')
        ax1.set_xticks(nu_array)
        ax1.set_xticklabels([ "{0:.0f}".format(i/1e6) for i in nu_array])
        ax2.set_xticks(nu_array)
        ax2.set_xticklabels([ "{0:.0f}".format(i/1e6) for i in nu_array])
        ax1.set_xlim(nu_array[0]*0.8,nu_array[-1]*1.2)
        ax2.set_xlim(nu_array[0]*0.8,nu_array[-1]*1.2)
...
#        print "C3:",printRes(*doPixelMC(C3,error,nu_array,calError=0.2,nu_ref=nu_ref,ax=ax3,order=order,title="C3"))
        ...
        print "NW1:",printRes(*doPixelMC(NW1,error,nu_array,calError=0.2,nu_ref=nu_ref,ax=ax1,order=order,title="NW1"))
        ...
        print "NW2:",printRes(*doPixelMC(NW2,error,nu_array,calError=0.2,nu_ref=nu_ref,ax=ax1,order=order,title="NW2"))
        ...
        print "H:",printRes(*doPixelMC(H,error,nu_array,calError=0.2,nu_ref=nu_ref,ax=ax1,order=order,title="H"))
        ...
        print "E:",printRes(*doPixelMC(E,error,nu_array,calError=0.2,nu_ref=nu_ref,ax=ax2,order=order,title="E"))
...
#6.785856666900907E-4
        print "X1:",printRes(*doPixelMC(X1,error,nu_array,calError=0.2,nu_ref=nu_ref,ax=ax2,order=order,title="X1"))
        ...
        #7.59771827938236E-4
        print "X2:",printRes(*doPixelMC(X2,error,nu_array,calError=0.2,nu_ref=nu_ref,ax=ax2,order=order,title="X2"))
        ...
        print "S:",printRes(*doPixelMC(S,error,nu_array,calError=0.1,nu_ref=nu_ref,ax=ax2,order=order,title="S"))
